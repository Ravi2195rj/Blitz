  ...
  behavior ThreadManager
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- until one is available.
          var availableThread: ptr to Thread
          threadManagerLock.Lock()
          while freeList.IsEmpty()
            threadBecameFree.Wait(&threadManagerLock)
          endWhile
          availableThread = freeList.Remove()
          (*availableThread).status = JUST_CREATED
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      var
        ...
        SystemStackTop:int
        EntryPoint:int
        OpenFilePointer: ptr to OpenFile
        temp:int
      ...
      newAddrSpace.Init()
      OpenFilePointer= fileManager.Open(&kernalFileName)
      if OpenFilePointer == null
        return -1
      endIf

      EntryPoint = OpenFilePointer.LoadExecutable(&newAddrSpace)
      frameManager.ReturnAllFrames(&((*currentThread).myProcess.addrSpace))
      currentThread.myProcess.addrSpace = newAddrSpace
      fileManager.Close(OpenFilePointer)
      (*currentThread).isUserThread = true
      UserStackTop = newAddrSpace.numberOfPages*PAGE_SIZE
      SystemStackTop = (& currentThread.systemStack[SYSTEM_STACK_SIZE-1]) asInteger
      newAddrSpace.SetToThisPageTable()
      BecomeUserThread(UserStackTop, EntryPoint, SystemStackTop)
      return 3000
    endFunction
